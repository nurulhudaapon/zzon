...
export default function JsonZonConverter() {
  ...
  const loadExample = (example: (typeof examples)[0]) => {
    setJsonValue(JSON.stringify(example.json, null, 2));
    setShowExamples(false);
  };

  return (
    <div className="h-screen w-screen flex flex-col md:flex-row p-0 md:p-4 bg-gray-50 dark:bg-gray-900">
      {/* JSON Editor */}
      <div className="w-full md:w-1/2 h-1/2 md:h-full relative pr-0 md:pr-2">
        <div className="absolute top-0 left-0 right-0 z-10 flex justify-between items-center px-4 py-2 bg-gray-700 rounded-t-none md:rounded-t-lg mr-0 md:mr-2">
        ...
        {jsonError && (
          <div className="absolute bottom-0 left-0 right-0 p-2 bg-red-900 bg-opacity-90 text-white text-sm font-mono rounded-b-none md:rounded-b-lg">
            {jsonError}
          </div>
        )}
      </div>
      ...
      {/* ZON Editor */}
      <div className="w-full md:w-1/2 h-1/2 md:h-full relative pl-0 md:pl-2">
        <div className="absolute top-0 left-0 right-0 z-10 flex justify-between items-center px-4 py-2 bg-gray-700 rounded-t-none md:rounded-t-lg ml-0 md:ml-2">
          <div className="text-white font-semibold flex items-center gap-2">
            <Image
